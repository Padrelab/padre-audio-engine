...
class Audio {
 public:
  ...
  static void AUDIO_LOG_IMPL(uint8_t level,
                             const char* path,
                             int line,
                             const char* fmt,
                             Args&&... args) {
...
class ps_ptr {
 public:
  ...
  bool alloc(const char* name = nullptr) {
    (void)name;
    reset();
...
class ps_ptr<char> {
 public:
  ...
  bool alloc(const char* name = nullptr) {
    ...
    reset();
    cap_ = 64;
    buf_ = static_cast<char*>(malloc(cap_));
    if (buf_ == nullptr) {
      cap_ = 0;
      return false;
    }
  ...
  void appendf(const char* fmt, ...) {
    ...
    if (buf_ == nullptr && !alloc()) return;
    const size_t prefix_len = strlen(buf_);

    va_list args;
    va_start(args, fmt);
    va_list copy;
    va_copy(copy, args);
    const int add_len = vsnprintf(nullptr, 0, fmt, copy);
    va_end(copy);
    ...
    const size_t need = prefix_len + static_cast<size_t>(add_len) + 1;
    if (!reserve(need)) {
      va_end(args);
      return;
    }

    vsnprintf(buf_ + prefix_len, cap_ - prefix_len, fmt, args);
    va_end(args);
  }
 ...
 private:
  bool reserve(size_t need) {
    if (need <= cap_ && buf_ != nullptr) return true;

    size_t new_cap = cap_ == 0 ? 64 : cap_;
    while (new_cap < need) {
      new_cap *= 2;
    }

    char* next = static_cast<char*>(realloc(buf_, new_cap));
    if (next == nullptr) return false;

    buf_ = next;
    cap_ = new_cap;
    if (buf_[0] == '\0') {
      // keep valid C-string invariant
    }
    return true;
  }
...
#ifndef PADRE_DISABLE_FAKEFRAME_FILTER
#define PADRE_DISABLE_FAKEFRAME_FILTER 1
#endif
...
#if PADRE_DISABLE_FAKEFRAME_FILTER
    int frameLen = 1;
#else
    // Skip fake frames
    ...
    *bytesLeft -= (fhBytes + siBytes);
    ...
    if (m_MP3DecInfo->mainDataBytes >= m_MP3DecInfo->mainDataBegin) {
        ...
        *bytesLeft -= (m_MP3DecInfo->nSlots);
        ...
        inbuf += m_MP3DecInfo->nSlots;
        *bytesLeft -= (m_MP3DecInfo->nSlots);
        if (underflowCounter < 4) { return MP3_MAIN_DATA_UNDERFLOW; }
        MP3ClearBadFrame(out16);
...
struct Esp32Mp3Context {
  static constexpr size_t kInputBufferSize = 8192;
  static constexpr size_t kMaxFrameSamples = 1152 * 2;

  Audio audio;
  MP3Decoder decoder;

  uint8_t input_buffer[kInputBufferSize] = {0};
  size_t input_begin = 0;
  size_t input_end = 0;
  bool source_eof = false;
  bool sync_locked = false;

  int16_t frame_buffer[kMaxFrameSamples] = {0};
  size_t frame_samples = 0;
  size_t frame_offset = 0;

  Esp32Mp3Context() : decoder(audio) {}

  size_t inputAvailable() const { return input_end - input_begin; }
  uint8_t* inputPtr() { return input_buffer + input_begin; }

  void compactInput() {
    if (input_begin == 0) return;

    const size_t available = inputAvailable();
    if (available > 0) {
      memmove(input_buffer, input_buffer + input_begin, available);
    }
    input_begin = 0;
    input_end = available;
  }
  ...
  void consumeInput(size_t bytes) {
    const size_t available = inputAvailable();
    const size_t consume = std::min(bytes, available);
    input_begin += consume;
  ...
  size_t appendInput(padre::IAudioSource& source) {
    compactInput();
    const size_t room = kInputBufferSize - input_end;
    if (room == 0) return 0;

    const size_t read_bytes = source.read(input_buffer + input_end, room);
    input_end += read_bytes;
    if (read_bytes == 0 && source.eof()) source_eof = true;
    return read_bytes;
  }
};

constexpr size_t kMinSyncBytes = 4;
constexpr size_t kKeepTailBytes = 3;
constexpr size_t kMaxInitScanBytes = 256 * 1024;
constexpr int16_t kFrameSentinel = static_cast<int16_t>(0x5A5A);
...
bool parseMp3Header(const uint8_t* data, size_t available, ParsedMp3Header* out) {
  if (data == nullptr || out == nullptr || available < 4) return false;
  ...
  const uint8_t version_id = static_cast<uint8_t>((b1 >> 3) & 0x03u);
  const uint8_t layer_id = static_cast<uint8_t>((b1 >> 1) & 0x03u);
  const uint8_t bitrate_idx = static_cast<uint8_t>((b2 >> 4) & 0x0Fu);
  const uint8_t sample_rate_idx = static_cast<uint8_t>((b2 >> 2) & 0x03u);
  const uint8_t padding = static_cast<uint8_t>((b2 >> 1) & 0x01u);

  if (version_id == 1) return false;  // reserved
  if (layer_id != 1) return false;    // Layer III only
  if (bitrate_idx == 0 || bitrate_idx == 0x0F) return false;
  if (sample_rate_idx == 0x03) return false;

  static constexpr uint16_t kBitrateMpeg1L3[16] = {
      0, 32, 40, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320, 0};
  static constexpr uint16_t kBitrateMpeg2L3[16] = {
      0, 8, 16, 24, 32, 40, 48, 56, 64, 80, 96, 112, 128, 144, 160, 0};
  static constexpr uint32_t kSampleRateMpeg1[3] = {44100, 48000, 32000};
  static constexpr uint32_t kSampleRateMpeg2[3] = {22050, 24000, 16000};
  static constexpr uint32_t kSampleRateMpeg25[3] = {11025, 12000, 8000};

  uint32_t sample_rate = 0;
  if (version_id == 3) {
    sample_rate = kSampleRateMpeg1[sample_rate_idx];
  } else if (version_id == 2) {
    sample_rate = kSampleRateMpeg2[sample_rate_idx];
  } else {
    sample_rate = kSampleRateMpeg25[sample_rate_idx];
  }
  if (sample_rate == 0) return false;

  const uint16_t bitrate_kbps =
      version_id == 3 ? kBitrateMpeg1L3[bitrate_idx] : kBitrateMpeg2L3[bitrate_idx];
  if (bitrate_kbps == 0) return false;

  const size_t frame_length = version_id == 3
                                  ? static_cast<size_t>((144000u * bitrate_kbps) / sample_rate + padding)
                                  : static_cast<size_t>((72000u * bitrate_kbps) / sample_rate + padding);
...
bool hasLikelyNextHeader(const uint8_t* data,
                         size_t available,
                         const ParsedMp3Header& current) {
  if (available < current.frame_length + 4) return true;

  ParsedMp3Header next = {};
  if (!parseMp3Header(data + current.frame_length,
                      available - current.frame_length,
                      &next)) {
    return false;
  }

  return next.version_id == current.version_id &&
         next.layer_id == current.layer_id &&
         next.sample_rate == current.sample_rate;
}

bool ensureInputBytes(Esp32Mp3Context& ctx, padre::IAudioSource& source, size_t min_bytes) {
  while (ctx.inputAvailable() < min_bytes && !ctx.source_eof) {
    const size_t read_bytes = ctx.appendInput(source);
    if (read_bytes == 0) break;
  }
  return ctx.inputAvailable() >= min_bytes;
}

size_t copyFrameToOutput(Esp32Mp3Context& ctx, int16_t* out_samples, size_t out_capacity_samples) {
  if (out_samples == nullptr || out_capacity_samples == 0) return 0;
  if (ctx.frame_offset >= ctx.frame_samples) return 0;

  const size_t available = ctx.frame_samples - ctx.frame_offset;
  const size_t to_copy = std::min(available, out_capacity_samples);
  memcpy(out_samples, ctx.frame_buffer + ctx.frame_offset, to_copy * sizeof(int16_t));
  ctx.frame_offset += to_copy;

  if (ctx.frame_offset >= ctx.frame_samples) {
    ctx.frame_offset = 0;
    ctx.frame_samples = 0;
  }
...
bool decodeNextFrame(Esp32Mp3Context& ctx, padre::IAudioSource& source, size_t* scanned_bytes) {
  for (size_t guard = 0; guard < 2048; ++guard) {
    if (!ensureInputBytes(ctx, source, kMinSyncBytes)) return false;
    if (ctx.inputAvailable() < kMinSyncBytes) return false;

    int32_t sync_offset = 0;
    if (!ctx.sync_locked) {
      sync_offset = ctx.decoder.findSyncWord(
          ctx.inputPtr(), static_cast<int32_t>(ctx.inputAvailable()));
    }

    if (sync_offset < 0) {
      if (ctx.inputAvailable() > kKeepTailBytes) {
        const size_t drop = ctx.inputAvailable() - kKeepTailBytes;
        ctx.consumeInput(drop);
        if (scanned_bytes != nullptr) *scanned_bytes += drop;
        ctx.sync_locked = false;
        continue;
      }

      if (ctx.source_eof) return false;
      if (ctx.appendInput(source) == 0) return false;
      continue;
    }

    if (sync_offset > 0) {
      const size_t drop = static_cast<size_t>(sync_offset);
      ctx.consumeInput(drop);
      if (scanned_bytes != nullptr) *scanned_bytes += drop;
      ctx.sync_locked = false;
      continue;
    ...
    if (!parseMp3Header(ctx.inputPtr(), ctx.inputAvailable(), &header) ||
        !hasLikelyNextHeader(ctx.inputPtr(), ctx.inputAvailable(), header)) {
      if (ctx.inputAvailable() > 0) {
        ctx.consumeInput(1);
        if (scanned_bytes != nullptr) *scanned_bytes += 1;
      }
      ctx.sync_locked = false;
      continue;
    }

    if (!ensureInputBytes(ctx, source, header.frame_length)) {
      if (ctx.source_eof) return false;
      continue;
    }
    if (ctx.inputAvailable() < header.frame_length) {
      if (ctx.source_eof) return false;
      continue;
    }

    int32_t bytes_left = static_cast<int32_t>(header.frame_length);

    constexpr size_t kProbeSamples = 8;
    for (size_t i = 0; i < kProbeSamples; ++i) {
      ctx.frame_buffer[i] = kFrameSentinel;
    }

    int32_t res = ctx.decoder.decode(
        ctx.inputPtr(), &bytes_left, reinterpret_cast<int32_t*>(ctx.frame_buffer));

    if (bytes_left < 0) bytes_left = 0;
    const size_t left = static_cast<size_t>(bytes_left);
    const size_t consumed = header.frame_length > left ? header.frame_length - left : 0;
    if (consumed > 0) {
      ctx.consumeInput(consumed);
      if (scanned_bytes != nullptr) *scanned_bytes += consumed;
    }

    if (res == MP3Decoder::MP3_STOP || res == MP3Decoder::MP3_NEED_RESTART) {
      ctx.sync_locked = false;
      return false;
    }

    if (res == MP3Decoder::MP3_NEXT_FRAME || res == MP3Decoder::MP3_MAIN_DATA_UNDERFLOW) {
      if (consumed == 0 && !ctx.source_eof) ctx.appendInput(source);
      if (ctx.source_eof && ctx.inputAvailable() == 0) return false;
      ctx.sync_locked = false;
      continue;
    }

    if (res < 0) {
      if (consumed == 0 && ctx.inputAvailable() > 0) {
        ctx.consumeInput(1);
        if (scanned_bytes != nullptr) *scanned_bytes += 1;
      }
      ctx.sync_locked = false;
    ...
    const size_t channels = static_cast<size_t>(ctx.decoder.getChannels());
    const size_t frame_samples_per_channel =
        static_cast<size_t>(ctx.decoder.getOutputSamples());
    if (channels == 0 || channels > 2 || frame_samples_per_channel == 0) continue;

    const size_t total_samples = frame_samples_per_channel * channels;
    const size_t probe_count = std::min(kProbeSamples, total_samples);
    bool has_pcm_output = false;
    for (size_t i = 0; i < probe_count; ++i) {
      if (ctx.frame_buffer[i] != kFrameSentinel) {
        has_pcm_output = true;
        break;
      }
    }
    ...
    ctx.frame_samples = std::min(total_samples, Esp32Mp3Context::kMaxFrameSamples);
    ctx.frame_offset = 0;
    ctx.sync_locked = true;
    return ctx.frame_samples > 0;
  }
...
bool Mp3Decoder::begin(IAudioSource& source) {
  stop();
  source_ = &source;

  auto* ctx = new Esp32Mp3Context;
  if (ctx == nullptr) {
    source_ = nullptr;
    return false;
  }
  ...
  while (scanned_bytes < kMaxInitScanBytes) {
    if (decodeNextFrame(*ctx, *source_, &scanned_bytes)) {
      first_frame_ok = true;
      break;
    }
    if (ctx->source_eof) break;
  }
  ...
  info_.channels = static_cast<uint16_t>(ctx->decoder.getChannels());
  info_.output_channels = info_.channels >= 2 ? 2 : 1;
  info_.sample_rate = ctx->decoder.getSampleRate();
  info_.bits_per_sample = 16;
  info_.total_pcm_frames = 0;
  info_.total_frames_known = false;
  info_.valid = info_.channels > 0 && info_.channels <= kMaxInputChannels &&
                info_.sample_rate > 0;
...
size_t Mp3Decoder::decode(int16_t* out_samples, size_t out_capacity_samples) {
  if (!running_ || decoder_ == nullptr || source_ == nullptr || out_samples == nullptr ||
      out_capacity_samples == 0) {
    return 0;
  }

  auto* ctx = static_cast<Esp32Mp3Context*>(decoder_);
  size_t produced = copyFrameToOutput(*ctx, out_samples, out_capacity_samples);

  while (produced < out_capacity_samples) {
    if (!decodeNextFrame(*ctx, *source_, nullptr)) {
      if (ctx->source_eof && ctx->frame_samples == 0) {
        ctx->input_begin = 0;
        ctx->input_end = 0;
        running_ = false;
      }
    ...
    const size_t copied = copyFrameToOutput(
        *ctx, out_samples + produced, out_capacity_samples - produced);
    if (copied == 0) break;
    produced += copied;
  }
...
bool Mp3Decoder::isRunning() const { return running_; }

const Mp3StreamInfo& Mp3Decoder::streamInfo() const { return info_; }
